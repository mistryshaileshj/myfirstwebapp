...
with col_left:
    ...
    fig_brand = px.bar(
        ...
        text=top10_brands["Revenue"].apply(lambda x: f"₹{x/1000:.0f}K"),
    )
    fig_brand.update_traces(textposition="outside", marker_line_width=0, marker_color="#d4601a")
    fig_brand.update_layout(
        coloraxis_showscale=False,
        plot_bgcolor="rgba(0,0,0,0)",
        paper_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(tickangle=-30, title=""),
        yaxis=dict(title="Revenue (₹)", gridcolor="#d4601a"),
